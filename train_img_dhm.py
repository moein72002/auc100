...
def test_ood_vs_cifar100(model):
    ...
    with torch.no_grad():
        for i, (x, y) in enumerate(tqdm(cifar100_ood_test_loader)):
            ...
            ood_logpz_list.append(type(logpz.item()))
            ood_logpz_list.append(type(logpz.item()[0]))
            ood_delta_logp_list.append(type(delta_logp.item()))
            ood_delta_logp_list.append(type(delta_logp.item()[0]))
